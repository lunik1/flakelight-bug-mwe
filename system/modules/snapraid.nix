  ...
  in mkIf cfg.enable {
    ...
    systemd.timers = with cfg; {
      snapraid-scrub = {
        ...
        timerConfig = { OnCalendar = sync.interval; };
      };
      snapraid-sync = {
      ...
      snapraid-scrub = {
        ...
        serviceConfig = {
          ...
          ExecStart = "${pkgs.snapraid}/bin/snapraid scrub -p ${
              toString cfg.scrub.plan
            } -o ${toString cfg.scrub.olderThan}";
          Nice = 19;
          ...
          ReadWritePaths = [ "/var/" ] ++ attrValues cfg.dataDisks;
        };
      ...
      snapraid-sync = {
        ...
        serviceConfig = {
          ...
          ReadWritePaths =
            ...
            let contentDirs = map dirOf cfg.contentFiles;
            in unique (map (mkPrepend "-") ([ "/var/" ] ++ [ "/media/par" ]
              ++ attrValues cfg.dataDisks ++ cfg.parityFiles ++ contentDirs));
